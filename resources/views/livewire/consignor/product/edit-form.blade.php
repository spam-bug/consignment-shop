<div>
    <div class="mt-8 flex gap-2 px-4 sm:px-6 lg:px-8">
        <x-button
            variety="secondary"
            outline
            wire:click="updateStatus"
        >
            {{ $form->product->status === \App\Enums\ProductStatus::Listed ? 'Unlist' : 'List' }} Product
        </x-button>

        <x-button
            variety="secondary"
            outline
            x-on:click="$dispatch('open-modal', { identifier: 'add-stock-form', 'product': {{ $form->product }} })"
        >
            Add Stock
        </x-button>
        <x-button
            variety="secondary"
            outline
            x-on:click="$dispatch('open-modal', { identifier: 'add-damage-form', 'product': {{ $form->product }} })"
        >
            Add Damage
        </x-button>
    ...
    <form wire:submit="save">
                    ...
                    @for ($index = 0; $index < 5; $index++)
                        <div>
                            @if (isset($form->photos[$index]) && $form->photos[$index])
                                @if ($form->photos[$index] instanceof TemporaryUploadedFile)
                                    <div class="group relative h-32 w-32 overflow-hidden rounded">
                                        ...
                                        <div
                                            ...
                                            <button class="text-white" wire:click.prevent="removePhoto({{ $index }})">
                                                Remove
                                            </button>
                                        </div>
                                    </div>
                            ...
                            @endif
                            <form.error for="form.photos.{{ $index }}" />
                        </div>
                    @endfor
                </div>
        ...
        <div class="lg:-8 border-b border-gray-200 p-4 sm:p-6">
